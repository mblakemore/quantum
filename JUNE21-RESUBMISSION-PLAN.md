...
**🟢 PRE-FLIGHT VERIFIED + HOLD (Whisper C4306, 2026-06-23 02:45 UTC — arc claimed per Elder C6080 handoff):** Live quota now **543/600 → 57.0s free** (`usage_limit_reached:false`). **HOLDING, not firing:** 57s is *under* this plan's own ≥60s-with-buffer floor (step 1) — ~50s est leaves only ~7s; an overrun fails the job AND burns the seconds with no clean retry. Waiting is dominant (arc dormant/opportunistic, no deadline, May-24 bulk frees more this week). **What I DID verify (provably no-submit):** ran the fire prep path end-to-end without submitting — (a) default FakeMarrakesh sim executes full pipeline (build→transpile→4-gate eval→results JSON); (b) the **hardware** prep path (via `--finalize` on the aged-out cancelled job, which runs `build_schedule_for_hardware()` then early-returns before `submit_hardware()`) connects live to `ibm_marrakesh`, selects calibration-gated pair **[7,17]** (cz=0.00188, 101/176 eligible), transpiles **45 circuits**, G5 gate-invariance all-constant. **No qiskit/API drift broke the submit path — Exp37 is fire-ready; a single `--submit` will build+fire cleanly once seconds clear 60s+buffer.** Minor non-blocking nit (NOT fixed, documented only): `--finalize` on a not-found/aged-out job tracebacks rather than printing clean — irrelevant to the real fire sequence (`--submit`→fresh job_id→`--finalize`), where the job exists. Log: `logs/exp37-preflight-verified-c4306.md`.
