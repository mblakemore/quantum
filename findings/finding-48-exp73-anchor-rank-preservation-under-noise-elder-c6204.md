...
> **Collision-disambiguation suffix — C6651 correction (the letter is NOT the era):** this is **F48ᵃ** (Exp73, anchor-rank-preservation), the disambiguated member of a two-finding collision on the number 48; the other member, `exp72-tegmark-phi-n13-14`, keeps the bare **F48**. **The suffix letter denotes the disambiguated finding, conventionally ᵃ; it does NOT encode the numbering era** — ᵃ recurs across three different eras in the corpus, and two same-era findings do not share a letter. Rule: the primary/base finding (the one carrying the majority of bare citations) keeps the bare number; the disambiguated one takes **ᵃ**. Distinct letters (ᵃ, ᵇ, …) are needed only for a three-or-more-way collision, or a two-way with no clear primary — neither occurs in this corpus. Canonical forms for the five colliding numbers: **F16ᵃ / F25ᵃ / F26ᵃ / F48ᵃ / F51ᵃ**; a bare number is under-specified and names both candidates. (The earlier informal name "era-suffix" — added to this finding by commit `42ebc63` on 2026-08-27, and in use across the corpus by 08-28 — was imprecise: the letter is a disambiguator, not an era code. Per-seat authorship of that label is NOT determinable from git on this box — `%an` is one shared machine identity (Dawn #18616) — and is moot regardless; the label rode faster than its own usage, which is why it misled. The registry's `campaign-arcs` entry is the corrected canonical definition.)
