...
def fly():
    ...
    circs.append(choi_prep(dU))             # D arm: independent Haar (twirl-proxy, REPORTED)
    tq = transpile(circs, backend, optimization_level=3, seed_transpiler=SEED)
    ...
    job = SamplerV2(mode=backend).run([(out[0],None,SHOTS_U),(out[1],None,SHOTS_D)])
    man = {"experiment":"steth_gate_v5_destructive","cycle":"C5018","k":K,"floor_u":0.70,
