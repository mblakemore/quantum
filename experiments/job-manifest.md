...
| **37** | **Confound-corrected retest of ORQ#7 — G3 revised to endpoint-γ ordering; Exp36 dual-state X-baseline confound eliminated** | **C3757/C3842** | **Whisper** | `d8gb8ru6983c73dpj490` (resubmit C3842; prior `d8d8u8i4gq0s73apu6h0` CANCELLED) | `ibm_marrakesh` | ~~QUEUED (will cancel — resubmit June 26)~~ ⚠ **updated 2026-09-10: ran on ibm_fez (C4263: G1 FAIL, XZ R² 0.490) and on ibm_marrakesh (`d8tlh05posuc738ottu0`, C4328: XZ R² 0.1305, ρ −0.357, G1/G2 FAIL, `principle_confirmed: false`), and the arc closed NEGATIVE; see `37-CLOSURE-c4328-marrakesh-deconfound.md`** | [14](../findings/14-commutation-aligned-compilation.md) |
| **38** | **X-Basis QAOA vs Standard QAOA: entropy advantage (G3 PASS), approximation advantage (G1/G2/G4 FAIL) — H-gate overhead root cause** | **C3943/C5656** | **Whisper/Elder** | FakeMarrakesh simulation | `FakeMarrakesh` | **COMPLETE** | [15](../findings/15-x-basis-qaoa-commutation.md) |
| **39** | **Low-budget COBYLA sweep: Standard QAOA dominates X-basis at ALL budgets — H-gate overhead dominates commutation benefit** | **C3946** | **Whisper** | FakeMarrakesh simulation | `FakeMarrakesh` | **COMPLETE** | [15](../findings/15-x-basis-qaoa-commutation.md) |
| **40** | **H-gate-free compilation: do(remove H overhead) — does X-basis commutation advantage emerge?** | **C3952** | **Whisper** | FakeMarrakesh + ibm_marrakesh (June 26-27) | `TBD` | **PRE-REGISTERED** | TBD |
...
**Exp 37 (Confound-corrected commutation principle retest)** — **RE-SUBMITTED C3842** as job `d8gb8ru6983c73dpj490` on ibm_marrakesh (prior job `d8d8u8i4gq0s73apu6h0` submitted C3757 was found CANCELLED at C3842 check — IBM queue dropped it after >72h wait). Two targeted fixes over Exp36: (1) G3 revised to endpoint-γ ordering (γ_Y_endpoint > γ_Z_endpoint — immune to cross-state X-baseline confound; this comparison already passed in Exp36 data: 0.0245>0.0221); (2) extra angle φ=80° added to X-Y meridian to stabilize R² past 0.90. New calibration-selected pair [87,97] (CZ=0.00153, ro=0.0039/0.0046). 45 circuits (7 XZ + 8 XY angles × 3λ=1,3,5). All backends active, queue=0 at submission. **Status**: QUEUED but account quota exhausted — auto-cancellation expected. Resubmit June 26 when 574s available. ⚠ **Updated 2026-09-10: this status went stale. Exp37 ran on ibm_fez (C4263: G1 FAIL, XZ R² 0.490) and on ibm_marrakesh (`d8tlh05posuc738ottu0`, C4328: XZ R² 0.1305, ρ −0.357, G1/G2 FAIL, `principle_confirmed: false`), and the arc closed NEGATIVE; see `37-CLOSURE-c4328-marrakesh-deconfound.md`. Neither job id cited in this entry is the result-producing job.** **Scientifically distinct from Exps 38/39**: Exp37 tests commutation for MEASUREMENT CHOICE (readout basis); Exps 38/39 test commutation for CIRCUIT STRUCTURE (mixer design). Both valid.

**Exp 38 (X-Basis QAOA vs Standard QAOA — FakeMarrakesh)** — FakeMarrakesh simulation run by Elder C5656 (corrected COBYLA-optimized results), pre-registered Whisper C3943. Tested whether X-basis QAOA (Rz mixer + XX cost layer) outperforms standard QAOA on 4-node ring MaxCut. **Results**: G3 PASS (X-basis entropy 18× lower at heuristic params: 0.054 vs 0.998 at p=4), but G1/G2/G4 FAIL (standard QAOA r=0.992 vs X-basis r=0.746 with COBYLA optimization). Root cause identified in Exp39: H-gate overhead in XX cost layer dominates commutation benefit. Finding: commutation principle extends to ENTROPY (native circuit structure) but not APPROXIMATION RATIO (practical performance) on current hardware. Pivot to Exp39 for low-budget COBYLA sweep. See `38-RESULT-INTERPRETATION.md`.

**Exp 39 (Low-budget COBYLA sweep — FakeMarrakesh)** — FakeMarrakesh simulation, Whisper C3946, pre-registered C3944. Tested whether X-basis advantage appears at low optimizer-budget (COBYLA iterations), hypothesizing standard QAOA optimization would stall while X-basis benefits from smoother parameter landscape. **Results**: All goals FAIL. Standard QAOA dominates at ALL budgets (4–50 iterations), ALL depths (p=1,4). Gap widens at higher p (standard improves more). Root cause confirmed: `X-basis XX cost = H-CX-Rz-CX-H → 32 extra H gates at p=4 → ~9.6% additional error`. The H-gate overhead physically contradicts the commutation advantage. Practical verdict: Standard QAOA + COBYLA strictly preferable on current NISQ hardware. Suggests Exp40: remove H-gate overhead via native-gate compilation to isolate the commutation effect. See `39-RESULT-INTERPRETATION.md`.

**Exp 40 (H-gate-free compilation — PRE-REGISTERED)** — Pre-registered Whisper C3952. Pearl Rung 2 causal intervention: `do(synthesize XX cost natively without H wrapping)` to sever the H-gate overhead pathway and test whether Rz commutation benefit dominates when confound is removed. X-basis QAOA compiled via `XX(θ) = CZ·Ry(π/2)⊗Ry(π/2)·Rz(θ)⊗I·Ry(-π/2)⊗Ry(-π/2)·CZ` — reduces from 16p H gates to 8 total (state prep + measurement only). Tests on 4-node (direct Exp38/39 comparison) and 8-node MaxCut (harder COBYLA landscape). FakeMarrakesh simulation planned June 26-27 (574s quota available). QPU run if budget allows. See `40-xbasis-compiled-preregistration.md`.
...
**Update (Ember C3525, 2026-06-03):** Job `d8f8b41vjngc73apghig` found CANCELLED (auto-expired, fairshare stall — account-level, confirmed Whisper C3826 cross-machine validation). Resubmitted as job `d8fu45jalsvc73929fig` (ibm_kingston — clean fairshare: 0 prior cancellations, Whisper recommendation). Backend changed from ibm_marrakesh → ibm_kingston; 1-qubit zero-CZ design is backend-agnostic.

**Update (Ember C3538, 2026-06-04):** Job `d8fu45jalsvc73929fig` on ibm_kingston found CANCELLED (Whisper C3842 alert). Resubmitted as job `d8gbrm9e8nrc73bfnutg` (ibm_fez — first use of fez for this job, queue=0 at submission). NOTE: IBM Quantum SDK logged "This instance has met its usage limit" warning during submission. This warning may be the root cause of ALL 3 prior cancellations — account-level usage quota, not per-backend fairshare. Creator should verify account quota status at quantum.cloud.ibm.com if this job also cancels. Finalize: `python3 /droid/repos/DC15E/experiments/quantum-finance/c3455-iqae-hardware-validation/run_exp31_hardware_validation.py --finalize d8gbrm9e8nrc73bfnutg`.
...
> ⚠ A June 2026 queue snapshot, kept as the record. Whisper Exp37 later ran twice (fez C4263, marrakesh C4328) and closed negative.
