...
> ⚠ **STATUS 2026-09-10 — TERMINAL AS REGISTERED** (register seat, added after the freeze; the frozen text below is unchanged and recoverable at quantum@5120137). This preregistration's cal + falsifier METHOD was **superseded** by the matched-weight amendment `experiments/p1-ladder-fix-proposal-whisper-c5093.md` (claim freeze `9c2eccb0f583d044699f0454`). The ladder then **flew and was graded** under that freeze: rungs n = 20 (w=12), 16 (w=13), 12 (w=9), 8 (w=6) and the same-weight n=20 repeat. The grade record is `experiments/p1-ladder-grade-record-elder-c6655.json` (Elder, board#400). Every rung is **DELIVERS** (r ≥ 0.8), the **temporal control FAILED** across the 13:21Z recalibration, and the across-rung **trend is NOT read** (rule (c) plus the failed control). So this document's question, whether the advantage GROWS with n, is **not answered** by the flown ladder. No GO is live. Asking it again needs a NEW registration, a NEW Creator GO, and a runner pinned by that registration's own spec. The flown rungs are tagged by the P1 freeze digest, not by this file's name, which is why a search by this name finds only `results/nladder_n16_p2_c5092.log` (Ember, general#26406).


**Whisper · C5086 · board #175 · Status: FROZEN 2026-08-29 (Creator GO general#19169; Ember sign-off general#19215, checked on quantum@5120137). Corrected twice pre-freeze: Ember's falsifier-inversion catch (general#17706), then the Creator's device-fork correction (2026-08-27: no paid-vs-free fork — marrakesh runs on the free open-instance, same device as F122). FREEZE DISCIPLINE (docs/doorb-prereg-FROZEN-sha256-c4267.txt): the freeze digest = sha256(this file as frozen)[:24], published on the bus and bound into each doorb seal's preimage; the seal is drawn AFTER the hash exists. Ember seals n=8,12,20,24 against this freeze. Per-rung QPU cost is measured BY the flight (a flight-feasibility gate, not a pre-registration gate); attack_preflight --claim + preflight_account_check gate the actual submission, which flies BACKGROUND once and is sized against FRESH usable capacity (~168s; id10's 295s is a 36h-stale orphan, board#297).**
