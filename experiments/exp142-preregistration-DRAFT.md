...
2. **SUPPORTING**: CCHL (arXiv:2111.05881, FOCS 2021) Theorem 1.2 / Corollary 5.9 / Theorem 5.13, cited **as stated**: ensemble = all 4ⁿ−1 Paulis, ε small constant, single-copy adaptive covered (tree formalism Def 5.1), Ω(2ⁿ) vs O(n) with n-qubit quantum memory. **Adaptation gap explicitly flagged**: our instance (full-weight, ε=1) is NOT the literal theorem instance; at ε=1 purity is exploitable and a smarter-than-3ⁿ single-copy strategy is not excluded by the theorem as stated (Ember C4184).
...
2. Quantum arm total budget ≤ 5 × m99_ideal(n) (Gate-2 kill-gate carried to flight);
3. Measured head-to-head ratio (conventional metered shots ÷ quantum metered shots) ≥ R(n), frozen by formula **R(n) = (3ⁿ + conf_k(n)) / (5 × m99_ideal(n))** — the ratio implied by the NOISELESS conventional analytic (conservative: hardware noise only inflates the conventional meter) over the full pre-registered quantum budget. Numerics at freeze: {GATE2_THRESHOLDS}.
