...
3b. **ERROR-BAR SCHEMA (Elder C6594 rider, from the door (a) post-mortem — added pre-submission):** every measurement-derived quantity in this prereg and its grading (q-hat(n) from the calibration block, per-arm success rates, the headline ratio) is published WITH its uncertainty (exact CP CIs for rates; SE for q-hat), significant figures matched to the uncertainty, and any GATE evaluates against the CI's unfavorable edge, never the point. Device context: u readings on this hardware showed CV ~24% across hours and shot-noise-limited stability within a job (door (a) #7133/#7140) — the same-batch in-job calibration block is the correct design and its k>1 draws carry their spread into the record.
