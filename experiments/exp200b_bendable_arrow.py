#!/usr/bin/env python3
"""Exp200b — THE BENDABLE ARROW, refly: same circuits, 194's exact compilation, relative gates. C4893.

Exp200 NOT HELD on absolute bands: optimization_level=1 + naive layout pinned bad physical
qubits (base C(2us)=0.359 vs 194's 0.802) while the PHYSICS survived in relative form
(full-kill revival +0.231 at 15 sigma, dose-independence spread 0.016, meta-arrow decline
0.636/0.413/0.245). Fixes: (1) transpile exactly as Exp194 (optimization_level=3, no
scheduling override) so layout picks good qubits; (2) every gate RELATIVE to the in-job
base (the 195b->200 lesson completed: in-job baselines must also be the pricing basis).
...
def submit(backend_name, shots):
    ...
    man = {"exp": "200b", "slug": "bendable_arrow_b", "backend": backend_name, "shots": shots,
           "job_id": job.job_id(), "order": CIRCS,
           "prereg": {"anchors": "C(base_2) in [0.65,0.92] (194: 0.802, same compilation now); "
                                 "bases strictly decreasing in T",
                      "bend": "C_bend(theta,2us) monotone decreasing; C_bend(pi) <= 0.10; "
                              "|C_bend - C_base*cos(theta/2)| <= 0.10 per dose",
                      "bend_back": "C_unbend(pi,2us) - C_bend(pi,2us) >= 0.5*C_base(2us) at >=5 sigma "
                                   "(RELATIVE gate); max-min of C_unbend over theta <= 0.15",
                      "meta_arrow": "Rec(T)=C_unbend(pi,T)/C_base(T) non-increasing in T; "
                                    "Rec(2us) >= 0.70",
                      "gauges": "unbend coin P(1) <= 0.15 (record returned; 200 measured storage "
                                "residue); bend coin P(1) tracks sin^2(theta/2)/2 within 0.06",
                      "budget_check": "revival contrast ~0.6 vs floor ~0 (C4887 rule; trivial)"}}
    json.dump(man, open(os.path.join(HERE, "..", "results", "exp200b_bendable_arrow_manifest.json"), "w"), indent=1)
    print(f"submitted {job.job_id()} ({len(circuits)} circuits, {shots} shots)")


def decode():
    from run_exp66_qpu_partb import _get_ibm_service
    man = json.load(open(os.path.join(HERE, "..", "results", "exp200b_bendable_arrow_manifest.json")))
    ...
    print(f"Exp200b THE BENDABLE ARROW decode | job {man['job_id']} | 194 anchors: 0.802/0.657/0.457")
    ...
    a_ok = (0.65 <= cb[2] <= 0.92 and cb[2] > cb[4] > cb[8])
    ...
    back_ok = rev >= 0.5 * cb[2] and zrev >= 5 and (max(unb) - min(unb)) <= 0.15
    ...
    g_unb = all(r[f"unbend_{th}_2"]["coin_p1"] <= 0.15 for th in THETAS)
    ...
    json.dump({"job_id": man["job_id"], "results": {n: r[n] for n in CIRCS},
              ...
              open(os.path.join(HERE, "..", "results", "exp200b_bendable_arrow_decode.json"), "w"), indent=1)
    print("-> results/exp200b_bendable_arrow_decode.json")
