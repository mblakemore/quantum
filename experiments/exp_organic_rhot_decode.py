...
MAN = json.load(open(os.path.join(RES, "exp_organic_rhot_flight_manifest.json")))
NPHYS = 156
...
def main(stage):
    ...
    tags = ["d_lo", "d_mid", "d_hi"]
    if stage == "gate":
        for tag in tags:
            for m in (0, 1, 2):
                run_block(f"lad_{tag}", f"lad_{tag}", 40, fm=m)
        path = os.path.join(RES, "exp_organic_rhot_gate_shat.json")
    else:
        for tag in tags:
            run_block(f"t80_{tag}", f"t80_{tag}", 40, subs=MAN["subsample_pubs"]["t80"])
        path = os.path.join(RES, "exp_organic_rhot_t80_shat.json")
