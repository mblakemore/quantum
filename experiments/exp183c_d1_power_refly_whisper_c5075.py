...
Layout pinned across all circuits (the discriminand is "does WHICH qubit carries Y matter").
  ...
  python3 exp183c_d1_power_refly.py --selftest          # positive controls P1+P2 (must pass to fly)
  QPU_ACCOUNT_VAR=IBMQ_ALT4 python3 exp183c_d1_power_refly.py --submit
  python3 exp183c_d1_power_refly.py --decode
"""
...
# EXP183C (C5075): ARM A ONLY, per the frozen prereg docs/exp183c-d1-power-refly-PREREG-whisper-c5075.md.
# Arm B is deliberately NOT flown: its gate G2 requires the anomaly present at >=0.05 and it measured
# 0.0248 today, so an intervention on an absent phase is untestable AT ANY SHOT COUNT. Flying it would
# be spending on a known NO-TEST. The shot count is 50,000 (vs 8,000) which puts D1's MDE at 0.0197,
# below today's residual — that is what makes this flight a test rather than a repeat.
ORDER = [["base", c] for c in ARM_A]
...
def submit(backend_name):
    ...
    out = os.path.join(RESULTS, "exp183c_d1_power_refly_manifest.json")
...
def decode():
    ...
    mp = os.path.join(RESULTS, "exp183c_d1_power_refly_manifest.json")
    ...
    grade(e3_base, e3_corr, man["shots"],
          out_path=os.path.join(RESULTS, "exp183c_d1_power_refly_decode.json"),
