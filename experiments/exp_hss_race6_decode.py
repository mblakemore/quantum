#!/usr/bin/env python3
"""Exp-HSS RACE 4 — frozen decode (card docs/exp-hss-race6-prereg-FROZEN-whisper-c4981.md).
...
MAN = json.load(open(os.path.join(RES, "exp_hss_race6_flight_manifest.json")))
NPHYS = 156
...
def main(stage):
    ...
    lay = MAN["layouts"]
    out = {"card": "exp_hss_race6_decode", "cycle": "C4981", "substrate": "claude-fable-5",
    ...
    if stage == "gate":
        ...
        path = os.path.join(RES, "exp_hss_race6_gate_shat.json")
    else:
        run_block("race_n40", "race_n40", 40, subs=MAN["subsample_pubs"]["race_n40"])
        path = os.path.join(RES, "exp_hss_race6_race_shat.json")
