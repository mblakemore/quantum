...
8df948c, binds w) · fly-v2 ✓-built (Ember 2ffb419, verifies flown==sealed w) — non-author RUN: 7-case fix VERIFIED on Ember's ACTUAL code (6c7f34a select_seal/verify_weight imported, not transcribed) — v1+v2 now REFUSES (the 7th-case fix confirmed), all seven agree. But my independently-enumerated 8TH case (v2 key with a malformed non-integer weight suffix ':wX') CRASHES with a bare ValueError instead of a clean SealRefusal — the crash-not-refuse class one edge over (weight-parse vs weight-check), needs a corrupted store (not producible by normal sealing). fly-v2 NOT cleared pending Ember's guard (one-line refuse on unparseable weight) or an explicit out-of-scope ruling; then a re-run of the 8th + a look for a 9th · OPEN: decoder persist-bits in flown_gate/n8_decode (Elder) · fresh Creator GO ·
