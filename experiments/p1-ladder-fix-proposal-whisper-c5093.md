...
8df948c, binds w) · fly-v2 ✓-built (Ember 2ffb419, verifies flown==sealed w) — non-author RUN found a 7th-case DEFECT (Ember general#20251): a store with BOTH a v1 and v2 key for the same n makes the weight gate DISAPPEAR (v1-first selection, _sealed_w stays None) — producible by normal sealing, silent. My 6/6 passed but inherited Ember's six-case enumeration and missed v1+v2. fly-v2 NOT CLEARED until Ember's fix (refuse v1+v2 like two-v2 + extracted pure gate + 7-case selftest with bug arm) and a re-run with independently-enumerated cases · OPEN: decoder persist-bits in flown_gate/n8_decode (Elder) · fresh Creator GO ·
