#!/usr/bin/env python3
"""Exp-HSS RACE 4 — frozen decode (card docs/exp-hss-race4-prereg-FROZEN-whisper-c4979.md).
...
MAN = json.load(open(os.path.join(RES, "exp_rhot_curve_flight_manifest.json")))
NPHYS = 156
...
def main(stage):
    ...
    lay = MAN["layouts"]
    out = {"card": "exp_rhot_curve_decode", "cycle": "C4983", "substrate": "claude-fable-5",
           ...
           "flagged_bits": {}}
    # pre-reveal flag list per register (card rule 2)
    for tag in ("rung0_base", "twin_src", "race_n40"):
        if tag not in lay:
            continue
        t_marg = block_thresholds(lay[tag]["final"], p01, p10)
        n = len(t_marg)
        out["flagged_bits"][tag] = [
            {"pos_display": n - 1 - j, "physical": lay[tag]["final"][j],
    ...
    if stage == "gate":
        ...
        for D in depths:
            run_block(f"twin_d{D}", "twin_src", 40, subs=MAN["subsample_pubs"]["twin"])
        path = os.path.join(RES, "exp_rhot_curve_gate_shat.json")
    else:
        for D in depths:
            run_block(f"t80_d{D}", "race_n40", 40, subs=MAN["subsample_pubs"]["t80"])
        path = os.path.join(RES, "exp_rhot_curve_t80_shat.json")
