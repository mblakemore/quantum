...
=== MECHANISM (frozen rule 2: ACTIVE iff rate != 0 at >=5 sigma) ===""")
    ...
    # ---- IN-JOB RANGE ASSERT (frozen): a rate that moved between build and flight VOIDS ----
    rc = man.get("range_check", {})
    maxdur = rc.get("max_duration_us", 0)
    inrange = {}
    print(f"""
=== IN-JOB RANGE ASSERT (frozen: fitted_rate x max_duration >= target, else PRIMARY VOID) ===""")
    for k, q in enumerate(probes):
        r = [x for x in rows if x["kind"] == "time" and x["probe"] == q][0]
        reach = abs(r["rate"]) * maxdur
        inrange[k] = reach >= TH
        print(f"  q{q:<4} fitted {abs(r['rate']):.3f} deg/us x {maxdur:.2f} us = {reach:6.2f} deg "
              f"vs target {TH} -> {'REACHABLE' if inrange[k] else 'NOT REACHABLE (primary VOID)'}")
...
=== PRIMARY (frozen rules 3-4: fit-selected rung, TOST vs delta={DELTA} deg) ===""")
    ...
    for kind in ("time", "depth"):
        for k, q in enumerate(probes):
            ...
            fired = equiv and ctrl_ok and gate[k] and inrange.get(k, False)
            verdicts.append({"kind": kind, "probe": q, "sel_n": man["ladder_n"][j],
                             ...
                             "gate": bool(gate[k]), "in_range": bool(inrange.get(k, False)),
                             "FREE_GATE_CERTIFIED": bool(fired)})
            print(f"  {kind:>5} q{q:<4} rung n={man['ladder_n'][j]:<3} "
                  f"diff {d:>+7.2f} +/- {sed:>5.2f} deg  p_TOST {p_tost:.4f} "
                  f"equiv={equiv}  ctrl={ctrl_ok}  gate={gate[k]}  range={inrange.get(k,False)}  "
