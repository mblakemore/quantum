...
n=8; SHOTS=None; p0=0.5+2**-(n+1)
RAW=glob.glob('results/doora_flight6_raw_*.json')
if not RAW: print('RAW not landed yet - rerun after Ember banks'); sys.exit()
d=json.load(open(RAW[0])); c=d['pubs'][0]['c']
rm=json.load(open(glob.glob('results/doora_flight6_rowmap*.json')[0]))
cal_pos=set(rm['cal_positions']); sealed=rm['sealed_positions']; TOT=rm['total_rows']
SHOTS=rm["shots"]; shots=np.array([[int(b) for b in s] for s in c]).reshape(TOT, SHOTS, 16)
...
json.dump({'card':'doora_flight6_whisper_blind_decode','job':d['job_id'],'convention':{'pairing':pairing,'reverse':reverse},
 'f_cal':fcal,'u_hat':uhat,'tau_Q':tau,'n_ALT':nALT,'n_NULL':M-nALT,'decisions':dec},
 open('results/doora_flight6_whisper_blind_decode_c5073.json','w'),indent=1)
print('-> results/doora_flight6_whisper_blind_decode_c5073.json')
