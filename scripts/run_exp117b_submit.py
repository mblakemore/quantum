#!/usr/bin/env python3
"""run_exp117b_submit.py — Exp117b dense rungs (Whisper C4616).
Prereg: experiments/exp117b-dense-rungs-preregistration.md (FROZEN).
...
RUNGS = {"r1": 1.55, "r2": 1.7, "r3": 1.85, "r4": 2.0}
SHOTS_SWITCH = 14000
from exp117_extraction_sim import build_extract  # noqa: E402
SHOTS_NULL = 4000
SHOTS_CALIB = 6000
SHOTS_SENT = 2000
SHUFFLE_SEED = 4616
...
def main():
    ...
    ap.add_argument("--tag", default="exp117b")
    args = ap.parse_args()
    ...
    for rung, r in RUNGS.items():
        ...
        for t0 in (0, 1):
            ent.append((f"{rung}_extract_t{t0}", rung, "extract", t0,
                        build_extract(t0, d_a, d_b), SHOTS_SWITCH))
    ...
    jid = job.job_id()
    manifest = {
        "experiment": "exp117b-dense-rungs", "cycle": "C4616-whisper",
        "backend": args.backend, "tag": args.tag,
        "prereg": "experiments/exp117b-dense-rungs-preregistration.md",
        ...
        "gates": {"passive": "p+5SE<0.5 each", "retention_min": 0.80,
                  "therm_band": 0.10, "W1": "drop - 5SE > 0.05", "W2": "p1m_extract + 5SE < 0.5", "G_plus": 0.05, "G_recert": "p1m_meas - 5SE > 0.5"},
