#!/usr/bin/env python3
"""grade_exp108c.py — apply the FROZEN Exp108c grade rule (Whisper C4592).
...
MANIFEST = os.path.join(HERE, "..", "results", "exp108c_jobids.json")

import exp108b_native_thermal as m108  # noqa: E402
# FROZEN 108c overrides (prereg C4592) — applied before grading
m108.CALIB_BAND = (0.12, 0.47)
m108.THERM_BAND = 0.10
grade_from_counts, self_validate = m108.grade_from_counts, m108.self_validate
...
def main():
    ...
    print(f"=== Exp108c GRADE (job {man['job_id']}, chain {man['chain']}, "
          f"layout {man['layout']}) ===")
    ...
    with open(os.path.join(HERE, "..", "results", "exp108c_grade.json"), "w") as f:
        json.dump(g, f, indent=1, default=float)
    print("wrote results/exp108c_grade.json")
