#!/usr/bin/env python3
"""run_exp116_submit.py — Exp116 ICO engine: certified inversion (Whisper C4609).

Prereg: experiments/exp116-ico-engine-preregistration.md (FROZEN).
...
m108.SHOTS_SWITCH = 20000
m108.SHOTS_CALIB = 12000
m108.P_TARGET = 0.45
BIAS = 1.65   # live-chip published-T1 bias (friction report 02); LIVE ONLY

import run_exp108b_submit as sub  # noqa: E402  (imports AFTER overrides)

SHUFFLE_SEED = 4609      # pub-order shuffle only
...
def main():
    ...
    ap.add_argument("--tag", default="exp116")
    args = ap.parse_args()
    ...
    t1_a = rep[6]["T1_us"] * 1e-6 * BIAS
    t1_b = rep[8]["T1_us"] * 1e-6 * BIAS
    ...
    jid = job.job_id()
    manifest = {
        "experiment": "exp116-ico-engine", "cycle": "C4609-whisper",
        "backend": args.backend, "tag": args.tag,
        "prereg": "experiments/exp116-ico-engine-preregistration.md",
        "gates": {"calib_band": [0.35, 0.50], "passive_premise": "p_hat+5SE<0.5 each", "retention_min": 0.80,
                  "therm_band": 0.10, "inversion_win": "p1|- - 5SE > 0.5"},
        "overrides": {"CALIB_BAND": [0.35, 0.50], "P_TARGET": 0.45, "BIAS": 1.65,
                              "SHOTS_SWITCH": 20000, "SHOTS_CALIB": 12000},
