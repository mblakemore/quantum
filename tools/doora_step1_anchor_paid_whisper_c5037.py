...
def main(fly):
    ...
    # WhisperPaid — PAID pay-as-you-go instance, pinned BY CRN not by name.
    # (Two accounts both name an instance "open-instance"; only the CRN is unique.)
    WHISPERPAID = ("crn:v1:bluemix:public:quantum-computing:us-east:"
                   "a/65155eedeb8b464eadf55d101fb3c931:"
                   "27609585-d5b2-43cb-808d-2d47aeb87c05::")
    svc = QiskitRuntimeService(channel="ibm_quantum_platform",
                               token=os.environ["IBMQ_TOKEN"],
                               instance=WHISPERPAID)
    ...
    RATE = 1.60   # USD per QPU-second, IBM pay-as-you-go
    print(f"  PAID INSTANCE: {rem}s remaining = ${rem*RATE:,.2f} of exposure at ${RATE}/s")
    if rem < 20:
        sys.exit(f"REFUSE: only {rem}s (${rem*RATE:.2f}) left — too thin for anchor + flight")
    ...
    out = {"cycle": 5037, "seat": "whisper", "step": "re-fly v2 step 1 (anchor) — WhisperPaid, PAID instance",
    ...
    p = f"{REPO}/results/doora_step1_anchor_paid_n8_whisper_c5037.json"
