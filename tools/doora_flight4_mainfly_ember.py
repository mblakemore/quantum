#!/usr/bin/env python3
"""DOOR (a) FLIGHT-4 binder (Ember) — the pure-Q WIN test at M=80. Identical
pipeline to the flight-3 binder that ran clean (doora_flight3_mainfly_ember);
the ONLY science change is the sealed trial count 40 -> 80. Delta on flight-3:

  * commitment  d889caa5 -> c31a1c86  (fresh balanced n=8 M=80 seal, Ember)
  * M (sealed)  40 -> 80              (Elder criterion 76/80 frozen, gen#12276)
  * cal base    30000 -> 30032        (Elder: fresh base, cross-flight provenance)
  * TOTAL_ROWS  72 -> 112             (80 sealed + 32 cal; interleave rule same,
                                       floor((j+0.5)*112/80) — computed generically)
  Flight-3 provenance (for the record) — delta on the flight-2 binder was:
...
CAL_SEED_BASE = 30032      # Elder flight-4 ruling: fresh base (was 30000) for clean cross-flight provenance
TOTAL_ROWS = TRIALS_TOTAL + K_CAL          # 112
SECRETS = os.path.expanduser("~/.ember-doora-secrets.json")
SEAL_KEY = f"doora_deg2phase_v1:{N}"
EXPECTED_COMMITMENT = "c31a1c861a840318c1baaa07bbb802d4f18876f3dd56b5c8c0a9016b73a6cdaa"
...
def main(shots, fly, hash_only):
    ...
    print(f"DOOR (a) FLIGHT-4 — n={N}, {TRIALS_TOTAL} sealed + {K_CAL} cal "
    ...
    if not fly:
        ...
        mp = "results/doora_flight4_rowmap_ember.json"
    ...
    json.dump(row_map, open("results/doora_flight4_rowmap_ember.json", "w"), indent=1)
    print("  row map (public, with job) -> results/doora_flight4_rowmap_ember.json")
    return 0
