#!/usr/bin/env python3
"""DOOR (a) FLIGHT-5 binder (Ember) — the POWERED pure-Q WIN attempt at M=80.
Identical pipeline to the flight-4 binder; the ONLY change is S (shots/trial)
316 -> 632, passed at runtime via --shots. S is the binding lever Elder's
6/6-noise-class error analysis identified (gen#12307): all flight-4 errors were
within 1.4 per-trial-SD of tau, so halving per-trial variance (S x2) resolves
the true-side trials at ~99%/trial. S is NOT part of the commitment (the seal
is M=80 balanced labels + A + salt only), so the seal is structurally identical
to flight-4. Delta on flight-4:

  * commitment  c31a1c86 -> f75f7540  (fresh balanced n=8 M=80 seal, Ember)
  * S (shots)   316 -> 632            (--shots 632; binding lever, Elder gen#12307)
  * cal base    30032 -> 30064        (Elder gen#12316: +32 lineage, provenance)
  M/K/n/interleave all carry (112 rows, floor((j+0.5)*112/80), S-independent).
  Flight-4 delta on flight-3 was M 40->80; flight-3 provenance below:
...
CAL_SEED_BASE = 30064      # Elder flight-5 ruling gen#12316: +32 lineage f3=30000/f4=30032/f5=30064
TOTAL_ROWS = TRIALS_TOTAL + K_CAL          # 112
SECRETS = os.path.expanduser("~/.ember-doora-secrets.json")
SEAL_KEY = f"doora_deg2phase_v1:{N}"
EXPECTED_COMMITMENT = "f75f7540109472a09c409943f0253ced63149790b19ef13a576cdf53838e6622"
...
def main(shots, fly, hash_only):
    ...
    print(f"DOOR (a) FLIGHT-5 — n={N}, {TRIALS_TOTAL} sealed + {K_CAL} cal "
    ...
    if not fly:
        ...
        mp = "results/doora_flight5_rowmap_ember.json"
    ...
    json.dump(row_map, open("results/doora_flight5_rowmap_ember.json", "w"), indent=1)
    print("  row map (public, with job) -> results/doora_flight5_rowmap_ember.json")
    return 0
