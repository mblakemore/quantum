#!/usr/bin/env python3
"""DOOR (a) FLIGHT-6 binder (Ember) — RE-FLY of the flight-5 WIN with PUBLIC
PRE-COMMITMENT restored (Elder ruling gen#12353). Flight-5 won on the physics
(78/80) but its commitment was git-committed 24s AFTER the job (seal+flown in
one post-flight commit), so an auditor could not rule out post-hoc label choice
on the PUBLIC record. This re-fly fixes exactly that and nothing else: same S=632
powered design, same frozen 76/80. The fix is enforced by the new G-PUBLIC gate
(below), which REFUSES to submit unless the commitment commit is already on
origin — code, not habit. Delta on flight-5:

  * commitment  f75f7540 -> 72b8f60e  (FRESH balanced 40/40; f5 labels now public)
  * custody     G-PUBLIC gate added   (commitment must be on origin PRE-submit)
  * cal base    30064 -> 30096        (+32 lineage, provenance)
  S=632 / M / K / n / interleave all carry (112 rows, floor((j+0.5)*112/80)).
  Flight-5 delta on flight-4 was S 316->632; flight-3 provenance below:
...
N = 8
TRIALS_TOTAL = 80          # flight-4: doubled from flight-3's 40 (Elder crit 76/80)
K_CAL = 32          # amendment 3 (Elder): 658 SAMPLES/316 shots = 32 ROWS; floor-32 for A-diversity
CAL_SEED_BASE = 30096      # flight-6 re-fly: +32 lineage f3/f4/f5/f6 = 30000/30032/30064/30096
TOTAL_ROWS = TRIALS_TOTAL + K_CAL          # 112
SECRETS = os.path.expanduser("~/.ember-doora-secrets.json")
SEAL_KEY = f"doora_deg2phase_v1:{N}"
EXPECTED_COMMITMENT = "72b8f60e07d868ed2df9ffa640f8aff32d0aa8a3e62b727b3439ee05a56ea490"
...
def g_public_gate(expected_commitment):
    """G-PUBLIC (Elder ruling gen#12353) — refuse to SUBMIT unless the sealed
    commitment is PUBLIC on origin BEFORE the flight. This is the timestamp
    flight-5 lacked (seal+flown bundled into one post-flight commit), now
    enforced by CODE not habit so it cannot be forgotten. Three checks:
      (1) the commitment file has no uncommitted changes,
      (2) its COMMITTED content's commitment_sha256 == expected_commitment
          (the pushed object is the sealed one, not a later edit),
      (3) the commit that last touched it is an ancestor of origin/main
          (i.e. it is actually pushed, visible to any auditor pre-flight).
    Any failure aborts before a single QPU-second is spent."""
    import subprocess
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    cf = "experiments/doora_commitments/doora_commitment_n8.json"

    def git(*a):
        return subprocess.run(["git", *a], capture_output=True, text=True, cwd=root)

    dirty = git("status", "--porcelain", "--", cf).stdout.strip()
    if dirty:
        sys.exit(f"REFUSE G-PUBLIC: {cf} has uncommitted changes ({dirty!r}) — "
                 f"commit AND push the seal before flying.")
    head_blob = git("show", f"HEAD:{cf}").stdout
    try:
        committed = json.loads(head_blob).get("commitment_sha256")
    except json.JSONDecodeError:
        sys.exit("REFUSE G-PUBLIC: cannot read committed commitment file at HEAD.")
    if committed != expected_commitment:
        sys.exit(f"REFUSE G-PUBLIC: committed commitment {str(committed)[:16]} "
                 f"!= EXPECTED {expected_commitment[:16]} — push the SEALED object.")
    git("fetch", "origin", "main", "-q")
    commit = git("log", "-1", "--format=%H", "--", cf).stdout.strip()
    anc = git("merge-base", "--is-ancestor", commit, "origin/main")
    if anc.returncode != 0:
        sys.exit(f"REFUSE G-PUBLIC: seal commit {commit[:12]} is NOT on origin/main "
                 f"— push it BEFORE the flight (flight-5's exact failure).")
...
def main(shots, fly, hash_only):
    ...
    print(f"DOOR (a) FLIGHT-6 — n={N}, {TRIALS_TOTAL} sealed + {K_CAL} cal "
    ...
    if labels[:TRIALS_TOTAL].count("1") != TRIALS_TOTAL // 2:
        sys.exit(f"REFUSE G-BAL: labels not balanced {TRIALS_TOTAL//2}/{TRIALS_TOTAL//2} "
                 f"(got {labels[:TRIALS_TOTAL].count('1')} ones)")
    print(f"  [PASS] G-BAL     labels balanced {labels[:TRIALS_TOTAL].count('1')}/{TRIALS_TOTAL // 2} ones")
    ...
    row_map = {"total_rows": TOTAL_ROWS, "shots": shots,
               ...
               "pattern": f"sealed j at floor((j+0.5)*{TOTAL_ROWS}/{TRIALS_TOTAL}); cal fill rest seed-order",
    ...
    if not fly:
        ...
        mp = "results/doora_flight6_rowmap_ember.json"
        json.dump(row_map, open(mp, "w"), indent=1)
        print(f"  row map (public) -> {mp}")
        print(f"  CAL_BLOCK_SHA256={cal_hash}")
        return 0

    # G-PUBLIC — the last gate before spend: the commitment MUST be public on
    # origin before this flight (the flight-5 custody fix, enforced in code).
    g_public_gate(EXPECTED_COMMITMENT)
    ...
    json.dump(row_map, open("results/doora_flight6_rowmap_ember.json", "w"), indent=1)
    print("  row map (public, with job) -> results/doora_flight6_rowmap_ember.json")
    return 0
