...
- **How cold can it go — the boundary (not a finding).** The ICO resource **saturates**: cascading the cooled qubit back through the switch converges to p₁ ≈ **0.177** (its causal-order floor for these baths, computed from the exact map), not toward zero. Combining several cold qubits with a **classical majority vote** reaches ~**0.13** on-chip (Exp139b, 37.6σ colder than a single input) — but that is **classical entropy compression** that works on any biased qubit, **not** the ICO resource; it takes no F-number and no ledger row (the F94 fuel-mislocation guard). Full decomposition: `docs/ico-cooling-floor-and-concentration-boundary-whisper-c4720.md`.
