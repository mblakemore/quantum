...
1. **§3 stethoscope FLIGHT pre-registration** (scout GO, 3-DC verified). Theorem co-check ✅ (Elder
   C6562, 9/9, adaptive-robust, O(1)=2 exp/eigenvalue) and tolerance ✅ (θ*<13° drift-binding,
   co-batch) are DONE. **The one remaining input is empirical: a *measured* coherent-SPAM fraction
   (< 13°) on the target chip region** — a short characterization run, then the pre-reg writes itself.
   The §3(b) **retrofit** (two-copy overlap replacing a tomography block) is the safer, lower-SPAM
   *first* flight — a measured shot-bill delta on an existing grader (Ember offered to take it on a
   Creator go: a real hardware flight, not QPU-blocked).
