...
| 2 | **Classical cost map** (§1) | ✅ **DONE (v1.0)** | meter + bench + sweep → `results/classical_cost_map_v1.json`; [doc](classical-cost-map-phase1-whisper-c4971.md). **Rank PAPER-PINNED** (γ=0.23) **+ absolute bill BAND** (paper-anchored: n=40/t=48 = 31s best-C-allcore … 3h MATLAB; t=80 = 6.5h … months). **Statevector 2ⁿ DEMONSTRATED** (high-n n≥22 slope 0.50→theoretical 0.69; run_s 0.26/0.75/2.54 at n=24/26/28). MPS min-χ doubles ~every 4 qubits. **Variance column DONE** (Ember 2-of-2, quantum@ae6e271): sv reproduces (median ratio 0.96, CV 3%, 2ⁿ slope unchanged), n=16 jitter outlier bounded; honest caveat = same-box machine-relativity, not cross-silicon. **v1.0 COMPLETE.** |
...
1. **§3(a) channel-spectroscopy FLIGHT** — pre-reg DESIGNED ✅ ([prereg](exp-steth-a-flight-prereg-whisper-c4971.md),
   ...
   flight-as-designed misses the Pauli premise and said STOP. **TWIRLED RE-FLY FLOWN** (ibm_marrakesh,
   job d9g18nkinv1c73arvgfg, quantum@9eea11a) → **PARTIAL SUCCESS**: the Pauli-twirl fix **WORKED at
   n=1** — X arms agree (twocopy 0.733 vs conv 0.710 within 2SE) and the **two-copy Z-control is
