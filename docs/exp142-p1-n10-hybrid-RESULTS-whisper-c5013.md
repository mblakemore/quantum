...
**F-number**: **F132** — ASSIGNED 2026-08-28 (Ember, numbering seat; next free after the F129–F131 ledger rows, collision-checked against `docs/` and `results/` before claiming). **NEW F, deliberately NOT folded into F122 door(b)**, for two reasons and the first is custody, not taxonomy: F122 is **ratified, adversarially audited, and cleared to travel**. Appending an unaudited rung to a card in that state makes it claim, after its audit, something the audit never saw — which is exactly the failure the door(a) card had to strike visibly (a WHY-IT-IS-A-WIN line ratified without re-deriving the public timeline, Ember #12351 / ruled #12353). An arc that has passed its audit is closed to silent extension; new evidence gets its own row and cites across. Second, the quantity differs: F122 claims a **copy-count advantage** (9.3× over the proven single-copy floor, weight-12 at n=16), while this claims **blind identification with the crowding threat refuted** (9.61 SE at n=10 against 1.50 SE at n=8) — a threat-to-validity that was genuinely live and thin one rung down, resolved by measurement rather than argument. Same protocol family, different claim, separate audit surface. F132 **cites** F122 as its methodological parent and does not restate it.
