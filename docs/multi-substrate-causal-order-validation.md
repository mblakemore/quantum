...
1. **Complete the ion's three-number card.** The honest gap in §6-1: fly the capacity (R̄) and null-integrity (D) arms on IonQ (64 pubs: 32 `cap_sw` + 32 `cap_nu`). **Cost correction (C4966)**: an earlier estimate of ~$530 assumed 100 shots/pub, but the null-integrity gate |D| + 5·se_D < 0.10 is a *tightness* test that **fails on statistics alone at 100 shots** (5·se_D ≈ 0.125 > 0.10, regardless of the physics); the capacity gate R̄ − 5·se_R > 0.10 similarly needs the margin. A card that would actually certify needs **~400 shots/pub ≈ $2,050** (minimum viable) up to the full frozen 1,500 shots/pub ≈ **$7,700** at IonQ's $0.08/shot. This would upgrade IonQ from *witness + validated matched null* (already certified, §4.3/§6b) to full three-number PASS-CAUSAL, symmetric with Rigetti — a **rigor upgrade, not a new claim**, and priced accordingly for a budget decision.
