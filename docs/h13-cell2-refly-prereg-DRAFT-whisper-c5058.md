...
**G-BAND CARRIES FOUR CLAUSES, each demonstrated firing on real or constructed counterexamples** — three of the four exist because a seat found a seam in it after it was specified: (1) **axis consistency** (the three diagonals must agree on a single p — fires on the flown dephasing data, CE spread 0.737); (2) **band match** (realized p̂ inside the declared band); (3) **arm agreement** (both arms drew the same p — Ember's seam #9372, Elder's repro #9377: CE at 0.35 and CC at 0.65 are each isotropic and each in-band, so 1+2 alone PASS them); (4) **spread** (Elder #9382, non-optional: the realized p̂ must BE a distribution over the band — a constant p is a member of any band containing it, and a fixed p means W = 0, under which the ceiling does not degrade but **divides by zero**; fires at sd 0.0000 vs an expected 0.1155).

- **G-BAND (Elder #9284, four clauses above)**: the **realized p̂ distribution must match the declared band**. Because |C| = (1−p)·0.9276, every run measures its own injection strength **continuously**: `p̂ = 1 − |C|/0.9276`, with per-run resolution ±0.026–0.033 and a band **mean estimable to ±0.018 across 40 runs**. Gate: realized p̂ mean, edges and shape consistent with the declared [0.30, 0.70]. This catches a drawer who published one set of draws and applied another, at 0.018 resolution rather than as a binary.
...
The four Pauli components are **enumerated as separate circuits**, and the mixture is realized **by shot allocation**, not by a per-shot RNG draw. For a given (unit, basis), the component shot counts are computed once from **that unit's own p** and applied to **both arms identically**.
**CORRECTION (Ember #9378, caught by arithmetic on a number I supplied):** an earlier draft of this section quoted `{I: 692, X: 103, Y: 103, Z: 103}` as the allocation for *every* (unit, basis). **That is wrong — it is unit 0's allocation only**, encoding a single p = 0.4113 (X = p/4 × 1000 = 103, I = (1 − 3p/4) × 1000 = 692). Since **p is drawn per unit**, the allocation *necessarily varies by unit* and a single global figure would have contradicted the band itself. Measured across the first five units: p = 0.4113 → {692, 103×3}; 0.4170 → {687, 104×3}; 0.6187 → {536, 155×3}; 0.4441 → {667, 111×3}; 0.5015 → {624, 125×3}.
**Shot allocation sums EXACTLY to the declared 1000** (Ember #9381, Elder #9382): naive per-component rounding gave 692 + 103×3 = **1001** against a declared 1000; replaced with **largest-remainder allocation plus an assert**, verified on every cell (unit 0 {691,103,103,103}; unit 2 {536,155,155,154}; unit 4 {624,126,125,125}). The physics does not care and **the record does** — a pre-registered integer differing from a shipped one is how *"the flown design was the frozen design"* stops being checkable.

**The structural claim is unaffected and is the narrower one**: within any unit the allocation is **identical across arms** (verified on every unit), which is what makes the twirl arm-identical. The *value* differs by unit; the *arm-identity* does not.
