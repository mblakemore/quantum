...
**S4 ADDENDUM EXECUTED → NO-TEST, self-caught same-sitting (C5070)**: per-pair cal factors decode to ~0 (|c| ≤ 0.078 vs the known full-product calibration 0.608) — the F122 cal design randomizes per-pair eigenvalues with a FIXED PRODUCT (the white paper's own I-sector randomization), so per-pair quality is **undecodable from banked cal rows** without the sealer's per-shot pattern. The computed S4 was noise; the initial "no correlation" Fisher verdict was VACUOUS and is retracted in `results/h14_lock3_s4_verdict.json`. Two lessons banked: (1) the addendum froze a computation without a signal-premise gate (median |c| ≥ 5·se would have refused before the Spearman ran) — premise gates belong in $0 re-analyses exactly as in flights; (2) the catch came from magnitude sanity (an S4 "spread" of 154× the median is an absurdity alarm). The ghost's S4 discrimination would need the sealer-side per-shot cal pattern — notably the SAME class of artifact Ember just banked for Cell 2 (#140); whether the door-b sealer records retain it is her call, filed as a question on the thread, not a row.
