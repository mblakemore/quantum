...
| U3 | unfold | $0 | oracle | ✅ | ✅ | **YES — mechanistic proof of the two-copy advantage** | ✅ **DONE C5073**: conv arm is a structured honest-oracle (fresh random even-parity b/row). PIN: true-basis row odd-rate 0.083 vs wrong 0.500 (mechanism on flown silicon). Single-copy boundary MAXIMALLY MIXED by design (uncond w1 |tr| 0.07); b-conditioning recovers the eigenvalue (~0.99). The mixed sign IS b; two-copy squares it away → dig A's wall AND the advantage explained | unfold_U3_conv_boundary_c5073.json |
