...
**The floor** (the thing F119 never had — see §8). Chen, Gong, Ye, *"Optimal tradeoffs for estimating Pauli observables,"* FOCS 2024 (arXiv:2404.19105), prove that any protocol measuring c copies at a time with k qubits of memory needs Ω(2ⁿ/(cε²)) copies. At c=1, k=0 — a single-copy learner with no memory — this is **Ω(2ⁿ/ε²)**. Crucially, the paper's protocol class is defined (its Definition 5) as *"a (c, M) protocol … applies a **(possibly adaptively chosen)** measurement,"* and its lower-bound machinery is the learning-tree / Le Cam framework whose nodes branch on prior outcomes. The floor therefore holds against **adaptive** single-copy strategies, not only non-adaptive ones — the strong form of the comparison. (The same paper notes that non-adaptive *sign*-learning is a separate, still-open hard case; we do the unsigned task and stay inside that boundary — §7.)
