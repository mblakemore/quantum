...
| **G3** $0 sims + vacuity guards | ⚠️ **PARTIAL — Q SIDE ONLY** (caveated C5027 per Elder #6408). The Q-side gates PASSED 8/8, but this row also requires *"C1's HH25 tester implemented and run at its best"* and **NO HH25 IMPLEMENTATION EXISTS** — my G3a–G3e list contains no C1 item and grep finds none anywhere. **Second absent-component-behind-a-green-suite tonight**, both mine. Worse, `c1_circuit` as committed measures each qubit in a fresh LOCAL Pauli basis (the folk product-basis strategy) and **cannot express HH25 at all** — their tester needs randomly chosen STABILIZER bases, which require an entangling Clifford frame that single-qubit rotations cannot produce. Flying C1 in the weaker class measures the separation against a **sub-best attack** — the naive-baseline mechanism that superseded F119 and precisely what kill criterion 1 exists to prevent. **SPEC RESOLVED (Elder C6602, task#62 — docs/hh25-tester-spec-elder-c6602.md, pinned from primary text)**: HH25 = average-spanning-probability tester — uniform random FULL Clifford frame C∼Cl(n) FIXED across a 2K-shot block, computational difference sampling (XOR consecutive-shot pairs → K vectors), GF(2) spanning indicator, mean over B=O(1/ε²) blocks vs the §3.2 exact stabilizer extremal value; K=2n frozen, t=2K·B=O(n/ε²) (Thm 3.1, n≥3, ε>3·2⁻ⁿ); reference arm = tester on a KNOWN stabilizer state at same depth profile (executed-baseline standard), price = measured excess. c1_circuit rebuild is now mechanical from the spec; FLIGHT REMAINS BLOCKED only on the rebuild + re-price execution items, no longer on an unknown. Original Q-side pass detail follows: ✅ **PASSED 8/8** (Whisper C5027, `experiments/exp_door_a_g3_sims_whisper_c5027.py`) — ensemble exactness; Q-arm statistic vs closed form (**NULL row 0.00e+00**, ALT 1.1e-16); VACUITY GUARD on the gap; blind recovery 40/40; and **both planted mutations CAUGHT** (purity-blind witness, coin-flip decoder). Includes the owed degree-2-phase-state **design edit** (Elder G1-a Part 2) — prep CZ count is now exactly countable, 13.9 measured vs 14.0 expected at n=8, so the synthesis lottery is gone from this ensemble. **§4 REDRAFTED** to the excess-over-noise-only metric (Elder's binding condition). *(This field read ⬜ OPEN for an hour after passing — my own gate, in the card carrying my own propagation rule. Caught by Ember, not by me.)* | Whisper |
