...
- RULE (frozen; Elder computes, Ember signs): with per-qubit values unavailable, report the corrected floor as a BRACKET, never a point: rigorous lower bound (1 − max)^9 = **0.865**; typical at the used-median (1 − 0.00537)^9 = **0.953**; the sentinel read **0.9143** lies between → **the finding's rescue of the sentinel ("0.9143 IS the joint-readout floor") is INDETERMINATE from banked data and leans UNFAVOURABLE** (at the typical value the read fails the corrected floor by ~0.04). G_SENT was already recorded False at flight (`gates.G_SENT = false`, flat 0.95 bar). If the per-qubit calibration for those 9 qubits is ever recovered, substitute and settle; otherwise the sentinel line is re-worded to the bracket.
