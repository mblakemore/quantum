...
**FROZEN before any seal exists.** Each instance's seal binds the **SHA-256 of this file's frozen bytes** — the same primitive F122 bound (SHA-256 of content), one convention across both preregs. The binding value is not written *inside* this file (a file cannot state its own hash without changing it); it is computed on the final frozen bytes and announced on the bus, exactly as F122's `0acd1f8a` was. A git blob hash (SHA-1) is a `git cat-file` *locator* only and is **not** the commitment primitive — my earlier announcement of the blob hash as 'the prereg blob' was an error, caught by Ember #8376 before it could enter a seal preimage. Frozen precedes sealed precedes submitted; the ordering is checkable against git and bus timestamps.
