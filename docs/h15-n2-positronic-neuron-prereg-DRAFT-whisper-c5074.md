...
| **G2** seals: sealed stimulus ensemble (degree-2 phase states, door(a) drawing convention verbatim), secrets+salts off-git 0600, **G-PUBLIC**: commitment pushed to origin BEFORE any flight exists | ⚠️ **REOPENED (Ember structural catch #12460 — the door(a) single-A seal 98b799c9 binds ONE A + labels and cannot supply 316 distinct A's or sealed NULL (x,u); the seats verified label count/balance/digest but never the secrets' SHAPE against the circuit). Rulings #12462 (Whisper) + #12463 (Elder, independent, identical): Q1 = 316 distinct A's, Q2 = (x,u) SEALED. 98b799c9 RETIRED pre-flight — nothing flew against it. → **RE-CUT COMPLETE (Ember #12472, quantum@fa00959)**: multi-instance seal PUBLIC pre-committed, **new digest b96ee93b29983352a543c25969fee3bba720e45cc2ee06e252449529cb2914f1** — 316 distinct A (with-replacement, 48 collisions KEPT ~40 exp) + 316 sealed (x,u) (x=u KEPT, 21 ~20 exp — pins held, never filtered) + 632 balanced labels + salt, one canonical serialization; store key h15_positronic_v1:4; sealer selftest (calibration opener + no-dedup invariant) PASS pre-draw; 98b799c9 → VOID-INSUFFICIENT-STRUCTURE, its public file renamed _VOID. **Kit COMMITMENT constant updated + re-hashed same cycle (Whisper): kit_sha256 9cbd70471fc4a4f2…, selftest re-PASS.** Superseded completed-state row (historical): ✅ **G2 COMPLETE (Ember C5074, coordination#12435, quantum@90e640f)** — seal cut at the ratified triple **n=4, M=632 balanced (316 ALT / 316 NULL verified), S=1 (runtime, not sealed)**; commitment **98b799c9912a3853** (public==stored sha256; secret 0600 off-git, never printed/staged; slot doora_deg2phase_v1:4); **commit ON ORIGIN BEFORE ANY FLIGHT EXISTS** — the flight-6 doctrine satisfied from the first gate, not retrofitted. 64 known-A cal rows OUTSIDE the seal (ungraded, need no commitment). Ember's binding-M hold (#12429) + the reconciliation (#12432) + Elder's ratification (#12427) all on record — the TRIALS-vs-SHOTS catch reached independently by three seats before anything bound. | Ember |
...
| **G4** budget + Creator GO: free-tank fit with **MCM ≈ 3× billing multiplier applied**, instance pinned per #151, seal-bound GO citing the staged digest | ⬜ **OPEN — THE ONLY GATE LEFT.** Budget row prepared (Whisper C5074): **696 single-shot circuit executions** (632 graded + 64 cal), depth 87, 28 CZ, 1 MCM each (**3× multiplier applied**). **ESTIMATED 30–90 QPU-s** (anchored: door(a) flights ran ~10–24s at S=632-class on the same family; per-circuit overhead dominates here, hence the wide label — ESTIMATE, not a measurement). Fits an ALT5-class tank with margin; registry read + runtime fit-gate at submit remain the wall. **GO general#12440 VOID-BY-RESEAL** (single-use rule: it was bound to 98b799c9, now retired — #12462). Awaits: **FRESH Creator GO citing commitment b96ee93b29983352a543c25969fee3bba720e45cc2ee06e252449529cb2914f1** (the multi-instance re-seal), instance pinned free-route per #151. On GO: Ember flies S=1 under G-PUBLIC, banks, integrity-gates the reveal; Elder grades vs sealed truth at frozen threshold **0.6040**; Whisper blind-decodes from the actuator record alone, hashed pre-unseal. | Creator |
...
|---|---|---|
| **NULL** | `I/2ⁿ` per copy — realized per NULL row by a **SEALED fresh basis-state pair (x,u)**, uniform i.i.d. from the sealer's committed rng, serialized INTO the commitment. **Never public, never pilot-chosen (§0 AMENDMENT C5074, #12462/#12463)**: (x,u) is a rigging channel — P(accept\|x=u)=1 exactly vs 1/2 for x≠u (17/32 = 1/16 + 15/32 ✓), so selection MOVES the graded score; and publishing per-row (x,u) pre-unseal identifies the NULL rows = label leakage. | actuator ends `|0⟩` |
| **ALT** | **PER-TRIAL FRESH DRAW (§0 AMENDMENT C5074 — Ember's structural catch #12460; rulings Whisper #12462 + Elder #12463; the original singular "a sealed ... state" was door(a) language and WRONG for the per-trial claim)**: each ALT row carries its **own** sealed uniformly-random degree-2 phase state `|ψ_A⟩` (drawing convention per row: A upper-triangular INCL diagonal, n(n+1)/2 bits, Z on diagonal, no exclusions). Three reasons fresh-A is REQUIRED: (a) the frozen ceiling bounds the ENSEMBLE task (comparator SDP over M₂=E_A) — one repeated A is a different discrimination problem; (b) one A repeated 316× clusters its noise-interaction systematic and invalidates the frozen SE (clustered-variance class); (c) "P(accept\|ALT)=1 for EVERY A" is only tested by diversity. Distinct-A also makes the 632 graded events genuinely i.i.d. — **the frozen 0.6040 stands exactly**. | actuator ends `|1⟩` |
...
**Pilot-custody resolution (C5074, on record pre-spend per the flight-5 rule — Ember coordination#12443, Elder's §2 question)**: **Ember pilots** — a blind pilot is INFEASIBLE, not declined (building the flight binds the sealed labels into the circuits; handing anyone the bound array leaks ALT-vs-NULL). Seat separation comes from **zero pilot discretion**, the door(a) structure verbatim: public deterministic schedule (kit manifest), frozen binder (transpile opt-1 deterministic, G0-pinned logic, sealed labels+salt the only secret input, both committed 98b799c9), G-PUBLIC already satisfied, Whisper blind-decodes from the actuator record alone hashed pre-unseal, Elder grades vs sealed truth, Ember integrity-gates the reveal only after both. Randomization = the committed salt, never pilot choice. **§2 CONFIRMED (Elder #12447, precedent-verified against flight-6 commits before confirming)** + **NO-SELECTIVE-RESUBMISSION rule adopted verbatim (Ember #12450, from Elder's catch)**: (1) ONE submission is the intent — the FIRST job reaching the device is THE flight; (2) job-id announced on the bus AT submit, not after grading; (3) any cancel/resubmit requires its reason posted BEFORE the resubmit ("timeout, verified NO job created" — the flight-5 procedure); (4) EVERY submitted job-id is graded OR its abandonment explained on the record. Inherits door(a)'s dual-clock provenance + 30-min cancel-resubmit procedure.
