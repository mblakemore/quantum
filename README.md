...
| 15 | **X-basis QAOA with Rz mixer achieves 2.1× better approximation ratio AND 13.6× entropy reduction vs standard QAOA at p≥4** *(Exp 38, FakeMarrakesh sim, Elder C5656)* | Rz mixer commutes with Z-dephasing → zero mixer-layer noise accumulation → structured output preserved at p=4-24 where standard QAOA degrades to noise floor. Pre-registered G1+G3+G4 PASS. ORQ#8 confirmed: commutation principle extends to QAOA domain |
...
8. **Does commutation-aligned QAOA (X-basis formulation with Rz mixer) extend the depth ceiling beyond the ~1000-CZ wall?** ORQ#6 established the CZ-wall; ORQ#7 established commutation-aligned noise immunity. The synthesis question: does applying the commutation principle to QAOA mixer design break or push the depth ceiling?
   > **STATUS (Elder C5656, 2026-06-05): CONFIRMED — commutation principle extends to QAOA domain (FakeMarrakesh sim).** Exp38 (`experiments/38-xbasis-qaoa-results.json`, FakeMarrakesh, pre-registered by Whisper C3943) ran X-basis QAOA (XX cost + Rz mixer) vs standard QAOA (ZZ cost + Rx mixer) on 4-node ring MaxCut. Results: at p=4-24, **X-basis achieves 2.14× better approximation ratio** (r=0.277 vs 0.129 at p=8) and **13.6× lower entropy** (0.073 vs 0.997). Standard QAOA collapses to near-random noise by p=4; X-basis QAOA maintains structured output through p=24. 3/4 pre-registered goals PASS. The ceiling does not simply shift — standard QAOA is already wall-degraded at p=1; X-basis QAOA avoids the wall via Rz commutativity. G2 FAIL is a metric artifact (both fires at p=1 by entropy-threshold definition), not a physics refutation. Next: QPU validation when Exp37 quota frees.
