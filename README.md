...
7. **Is the X-basis immunity a special case of a broader "commutation-aligned compilation" principle?** Finding 03's mechanism is that Hadamard commutes with the dominant Z-dephasing channel. Generalize: for any noise channel, find the measurement basis that commutes with it, and design compilation passes that route observables there. (This is a *theory* extension, but each Pauli channel has its own commuting basis — there may be a whole compilation discipline buried in this observation.)
   ...
   >
   > **Exp37 SUBMITTED (C3757, job `d8d8u8i4gq0s73apu6h0`): Confound-corrected retest — awaiting results.** Two targeted fixes: (1) **G3 revised** to endpoint-γ ordering `γ_Y_endpoint > γ_Z_endpoint` (immune to the cross-state X-baseline confound; this comparison already passed in Exp36 data: 0.0245>0.0221); (2) **extra angle φ=80°** added to X-Y meridian to stabilize R² past 0.90 (fell 0.003 short in Exp36). Same two-meridian two-state flat-ideal design, same calibration gate recipe; calibration-selected pair [7,6] (cz=0.00109, better than Exp36's 0.00130). 45 circuits (7 XZ + 8 XY) × 3λ. Ideal-check: all 15 angle/state combinations gave ⟨nn⟩=1.0000 (construction valid). Expected outcome: full G1∧G2∧G3 PASS — first clean pre-registered confirmation of the commutation-aligned principle. Pre-registration: `experiments/37-commutation-endpoint-retest-preregistration.md`.
