...
7. **Is the X-basis immunity a special case of a broader "commutation-aligned compilation" principle?** Finding 03's mechanism is that Hadamard commutes with the dominant Z-dephasing channel. Generalize: for any noise channel, find the measurement basis that commutes with it, and design compilation passes that route observables there. (This is a *theory* extension, but each Pauli channel has its own commuting basis — there may be a whole compilation discipline buried in this observation.)
   ...
   >
   > **Exp37 RE-SUBMITTED (C3842, job `d8gb8ru6983c73dpj490`): Confound-corrected retest — QUEUED on ibm_marrakesh, awaiting execution.** Prior job `d8d8u8i4gq0s73apu6h0` (C3757) was found CANCELLED after >72h wait (IBM queue dropped it). Resubmitted C3842 with new calibration-selected pair [87,97] (CZ=0.00153, ro=0.0039/0.0046). Two targeted fixes: (1) **G3 revised** to endpoint-γ ordering `γ_Y_endpoint > γ_Z_endpoint` (immune to the cross-state X-baseline confound; this comparison already passed in Exp36 data: 0.0245>0.0221); (2) **extra angle φ=80°** added to X-Y meridian to stabilize R² past 0.90. 45 circuits (7 XZ + 8 XY angles × 3λ=1,3,5). Expected outcome: full G1∧G2∧G3 PASS — first clean pre-registered confirmation of the commutation-aligned principle. Status update (C3860, 2026-06-04): Job `d8gb8ru6983c73dpj490` confirmed QUEUED on ibm_marrakesh (backend active, queue=0). 600s quota window reset Jun 4 19:00 UTC — job eligible to run. Pre-registration: `experiments/37-commutation-endpoint-retest-preregistration.md`.
