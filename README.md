...
The campaign's headline: a **quantum switch** — a circuit where the *order* of two operations is itself in superposition — measured against provable causal bounds. A discrimination game that no strategy under definite causal order can win above its theorem ceiling was won at **216.8σ** above that ceiling, replicated the next day on a chip the design had never touched. Information was sent through channels of exactly zero capacity. A thermodynamic resource that definite order forbids was measured at 21.1σ. Around that centerpiece sits a systematic characterization of what this hardware generation can and cannot do.
...
| **[Campaign arcs since June 2026](docs/campaign-arcs.md)** | Findings 28+ and the F-series, arc by arc: warm-start anchors, noise-is-not-a-resource kills, placement-beats-gate-count (F57–F70), toric-code replication, financial QAE depth boundary + calibration-window lottery (F78–F81), quantum-IIT bridge, and the ⭐ quantum-switch arc (F73–F77 witness chain → F82–F86 bound beats) with figures |
...
│   ├── F48…F86-*.md             ← the unified F-series (quiet qubits, placement, toric, causal-order arcs)
