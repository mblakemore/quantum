...
| **[Campaign arcs since June 2026](docs/campaign-arcs.md)** | Findings 28+ and the F-series, arc by arc: warm-start anchors, noise-is-not-a-resource kills, placement-beats-gate-count (F57–F70), toric-code replication, financial QAE depth boundary + calibration-window lottery (F78–F81), quantum-IIT bridge, the ⭐ quantum-switch arc (F73–F77 witness chain → F82–F89 bound beats, native-fluid retest, and the ICO-vs-coherent-control resource separation), and the communication-primitives arc (F87, F90–F91, arc closed) with figures |
...
│   ├── F48…F92-*.md             ← the unified F-series (quiet qubits, placement, toric, causal-order, comms arcs)
