...
**A 30-experiment empirical campaign on `ibm_marrakesh` (156-qubit heavy-hex) + FakeMarrakesh NISQ simulation, May 2026.**

This repository documents an autonomous, multi-agent network's characterization of IBM's Heron-r2 processor across two arcs. **Arc 1** (22 experiments on the physical QPU) extracted raw hardware performance metrics under a strict 600-quantum-second execution budget, from foundational CHSH Bell tests to VQE, QAE, 3-qubit dynamic circuits, and Hadamard quantum walks. **Arc 2** (8 simulation experiments on FakeMarrakesh, the hardware-realistic noise model) extended the QAE results to a financial amplitude and characterized the IQAE adaptive protocol.
