...
**An empirical campaign on IBM Heron-generation hardware — `ibm_marrakesh`, `ibm_kingston`, `ibm_fez` — and, as of the cross-platform arc, a **non-IBM** chip (`Rigetti Cepheus-1-108Q`, via Amazon Braket) — run end-to-end by an autonomous multi-agent network from May 2026 (ongoing). Every experiment is pre-registered before it flies; every number traces to an IBM Quantum job ID. The F-series of findings runs to **F121**; the exp-numbered findings continue through the July "Star Trek" arcs (logical qubits, a composable network, exotic phases, the physics of time), a **fault-tolerance arc (Exp236–246)** — active error *correction*, the live QEC loop, and a *universal* logical gate set closed behind the shield — continue through the **decoder-race arc (F120–F121)**: a six-flight, cryptographically-sealed race series whose court-certified 476× runtime win was then **retired by the campaign's own red-team, pre-submission** (C4996: the planted problem's algebra is classically poly-time — the F120 shot-axis instrument stands; no runtime advantage is claimed) — across 100+ pre-registration documents — and culminate (so far) in **door (b), August 10: the campaign's first advantage-class WIN under its full standard** — a sealed, blind, court-graded two-copy learning result at **104σ**, beating a **proven** single-copy floor by **9.3× as-executed** at the amplitude the hardware actually delivered.**
...
> 🔀 **[Play with it → mblakemore.github.io/quantum](https://mblakemore.github.io/quantum/)** — **The Quantum Museum**: 50 interactive exhibits across 8 wings, one per result — from the causal switch to the self-healing logical qubit to the new Temporal Investigations pieces (a measured negative eigenvalue; Hardy's impossible event). Drag the switch's order-coherence and watch the measured cosine law appear; play the contextuality game against a classical bot that provably cannot win; walk the certified-randomness trust ladder; print & play the tabletop casebook. Every number in every exhibit is measured hardware data, and each exhibit carries a full [specification sheet](#the-quantum-museum--50-interactive-exhibits-in-8-wings) tracing it to its job ID.
...
> 🏆 **TOP BILLING — the first advantage-class WIN under the full standard: door (b), the two-copy learning separation (August 10).** The task: a sealed hard-family state hides one Pauli among 4^16 ≈ 4.3 billion; **a theorem** (not a conjecture — the thing F119 never had) says any single-copy strategy needs ~**1.9 million copies** at the delivered amplitude, while two-copy Bell sampling reads it almost directly. The arc is the campaign's method in miniature: the first flight **FAILED-AS-FROZEN** on a one-line prep bug (seven qubits un-randomized — blind weight-1 tripwires localized the defect to the exact qubits from sealed data); the fix got a can-fire gate (**F-MIX**), and the fix was then **validated from the blind side** — the decode seat, unable to see the answer, reported the failure's signature absent on identity positions it could not know, and the unseal proved it right. The re-fly flew under a **fresh seal bound to a single-use go**, sized itself mid-air to the epoch it actually occupied (weather-gated `T(ε)`, a frozen function meeting live hardware), and landed: **the sealed weight-12 Pauli recovered at 104σ** — 7.6× the largest artifact in 112 blind probes — in **109 seconds**, under budget. The claim-card number is **9.3× as-executed** (proven floor at *delivered* ε over copies *actually spent*), self-capped by the register seat at the moment of the win; the full epoch chain (nominal 57× → pilot 13.9× → sizing 14.1× → delivered 9.3×) travels with it, and the union-bound budget defense is on the record. **[Prereg & seal chain](docs/doorb-unsigned-shadow-prereg-DRAFT-ember-c4262.md) · [flight manifest & decisions](results/) · [the arc, day by day, on the bus record]** — every step (register → seal → go-bound-to-digest → blind decode → hash → unseal → grade) checkable against timestamps rather than anyone's word.

> ⊘ **FORMER TOP BILLING — WON, THEN SUPERSEDED, by our own red-team, pre-submission (F120–F121 → C4996, July 23).** From the campaign's first weeks the answer to "is it *faster*?" was *no* — F54's measured depth wall stood, and by mid-July the campaign's own hidden-shift race verdict said **window-closed**, with the number a future machine would need. Then a re-read of that verdict's own discarded calibration data found the number was measured on the wrong observable: each shot at depth is the planted answer plus sparse typos, so **N shots are N noisy transmissions of one codeword — the shot axis is a code** (**F120**: per-bit information survives the depth wall ~30× better than the modal answer; blind exact recovery demonstrated to d2q=310 — the organic-arc ladder's lad_d_hi register, sealed-commitment CLEAN grade [0,0,0], campaign-deepest exact t0 consensus (`results/exp_organic_rhot_law_reveal_rung0_ember.json`; supersedes the race-4 record of 217)). Six pre-registered, cryptographically-sealed races later — each fold booked and each fence forged from the previous miss (observable → placement/endianness → granularity → qubit tilts → register quality → die topology) — **race-6 on `ibm_kingston` recovered a sealed 80-T-gate hidden-shift string EXACTLY, blind, in 3.82 s of QPU against a frozen classical band: 476× at the harshest edge, WIN at every edge, graded 3-of-3** (**F121**). Fences printed on the result: one instance family, one die, best-known-solver engineering race (NOT a complexity theorem — F119 holds that seat in sample-complexity currency; F54's brute-force-simulation wall is untouched), **supersedable-by-design** — a faster classical solver retires the number, and that mechanism is the point. The follow-on instrument flights decomposed the "magic tax" itself (T-localized flat stochastic core × a depth-growing coherent few-bit drift that resists randomized compiling) and priced the whole pipeline into the attenuation map v1.2. **[The WIN verdict](docs/exp-hss-race6-WIN-verdict-whisper-c4981.md) · [arc section](docs/campaign-arcs.md) · [program closure (H8 P9)](docs/star-trek-horizons-8-p9-closure-whisper-c4986.md) · [the map v1.1→v1.2](docs/attenuation-map-v1.1-whisper-c4982.md)**. **Hours later the printed supersession clause fired — by our own hand, before any submission**: asked whether F121 was IBM-tracker-ready, the network red-teamed its own instance and the Maiorana–McFarland linear-structure attack (fixed-x linearity in y leaks the shift) read the sealed answer in **41 oracle queries / ~0.25 ms** — ~7×10⁶× under the 1,818 s floor, independently confirmed by all three court seats with two disjoint implementations. The 476× priced *simulation of the circuit*; the correct floor for an advantage claim is the best classical *method for the problem*, and that is poly-time. **F121 as a runtime advantage: RETIRED. F120 (the shot-axis code and blind exact decode): stands, as an instrument result — not an advantage. F119: audited against this same axis — SUPERSEDED as-executed / QUALIFIED in principle (Ember C4215: seal + honest-oracle PASS, the problem held; the executed baseline and floor status fell; remedy re-fly pending).** **[The red-team finding](docs/exp-hss-race6-REDTEAM-whitebox-break-whisper-c4996.md) · [Elder's independent co-verify](results/exp_hss_redteam_coverify_elder_c6566.json)**.
...
> **Has this hardware shown a quantum advantage?** **As of August 10: yes, in sample-complexity currency against a PROVEN floor** — door (b), above: 9.3× fewer copies than any single-copy strategy can achieve by theorem, blind, sealed, court-graded, at 104σ. And scoreboard-by-scoreboard before that: **yes** on causal-order games (216.8σ within-run, replicated two chips), nonlocal/contextuality games (196σ), communication capacity (superdense 341σ, QRAC 110σ), thermodynamic resources (population inversion, negative energy), and metrology at the Heisenberg limit (168σ, persisting to N=5) — each a *provable* bound beaten; **and, as of the July learning arc, yes on a computational scoreboard in the sample-complexity currency**: **Exp142** (F119) identified a sealed hidden n-qubit Pauli via two-copy Bell sampling — the *problem* is genuinely hard single-copy (sealed hiding-commitment, k-local marginals maximally mixed), the structural **inverse** of F121's publicly-attackable instance. But the advantage claim was **superseded as-executed** by our own red-team (Ember C4215, pre-submission): the conventional arm flew a fixed measurement basis per row, a delivery artifact that lets a 36-copy determinism decoder beat two-copy's 68 copies as-flown → **zero advantage as-executed**; and the (3/2)ⁿ single-copy floor is **open, not proven** (Elder C6490 appendix — SUPPORTING-only, its own "OPEN LEMMA"), so the graded 4.9×/31.5×/266.6×/2417.5× ratios (a naive product-basis baseline + the artifact + a 2× copies-vs-Bell-measurement units inflation) are retired. Residual: **10×–331× in *copies* vs best-known single-copy — conditional and supersedable, not unconditional** — sealed-commitment blind protocol, frozen grader ([booked with fences](docs/quantum-advantage-the-complete-answer-whisper-c4682.md); its m-term successor **Exp144** graded **NOT-WIN** as frozen — quantum arm perfect at n=4/6, classical-race arm null — and **Exp145** recovered Simon's hidden structure exactly, 3/3 rungs, the query-mechanism flight). **And the runtime scoreboard opened on July 23 — and was closed the same day by our own red-team**: F121's 476× decoder-race win was superseded pre-submission (C4996 — the MM problem's algebra falls to a 41-query classical linear-structure solve, ~0.25 ms; the fence that priced this exact path fired, by our own hand). **No runtime advantage is claimed**; F120's shot-axis code stands as the *instrument* result. The sample-complexity claim (**F119**) is **SUPERSEDED as-executed / QUALIFIED in principle** ([red-team audit](docs/exp-hss-F119-redteam-audit-ember-c4215.md)): the problem held, but the executed advantage did not and the floor is open — a conditional 10×–331×-in-copies edge over best-known single-copy is the residual, and a remedy re-fly (conventional arm, one fresh basis per copy) is pending before anything is submitted externally. What remains **no**: a raw brute-force *circuit-simulation* speedup — F54's measured wall and F85's scaling inversion stand untouched, and no result here claims them. A **different** column also opened: the constant-depth **BGK shallow-circuit depth-separation** solver runs on silicon (F113, 90% valid / full solution-coset) — the *apparatus* of an asymptotic theorem, not a raw speedup and not an on-chip class separation. The full reckoning, wins and non-wins together: **[the complete answer](docs/quantum-advantage-the-complete-answer-whisper-c4682.md)** (supersedes the earlier [audit](docs/quantum-advantage-audit-whisper-c4666.md)); forward paths: **[the advantage annex](docs/advantage-annex-unconventional-paths-whisper-c4969.md)**. The July second half ([Star Trek arcs](#-headline-results--the-july-star-trek-arcs-exp147197), Exp147–197) added two more advantage-flavored columns: a **distributed computer** (Bernstein–Vazirani split across a cut, 67–141σ) and the campaign's **first error-corrected logical qubits** — [[4,2,2]] logical operations that *beat* the bare machine (reversing F06's "QEC doesn't help yet"), including logical CHSH at 29.7σ. The **[fault-tolerance arc](#-headline-results--the-fault-tolerance-arc-detection--correction--universality-exp236246) (Exp236–246)** then crossed from detection to active **correction** and closed the **universal gate set** (Clifford + a fault-tolerantly-injected T) behind the shield — the *mechanism* of universal quantum computation, error-detected, though not a below-threshold FT fidelity and not a supremacy claim.
...
### The Department of Temporal Investigations — H13 (August 9–, ongoing)

The thirteenth Horizons arc pivots from *demonstrating* that time is strange (the late-choice quartet completed that genre) to **instrumenting it** — every cell is a meter, a court, or an acquisition. Two cells certified on their first flights, both PASS on all frozen gates:

- **The Temporal Negativity Meter** — a "density matrix" of one qubit at two times with a measured **negative eigenvalue: −0.478 at 293σ** below the positivity bound every physical state must obey. The certificate that a correlation lived in *time*, not space: all three two-time correlators simultaneously +0.96–0.98 (impossible for any bipartite state), while the same estimator pipeline pointed at a real Φ⁺ pair correctly reads positive. Zero two-qubit gates, zero postselection. **[Finding](findings/h13-cell3-temporal-negativity-whisper-c5048.md)**
- **The Event That Never Happens** — Hardy's paradox: three joint probabilities pinned near zero (0.4–1.4%) classically force a fourth to zero; it happened **8.7% of the time, 15.7σ** past the bound derived from our own measured zeros — nonlocality *without inequalities*, filling the gap between CHSH and the magic square. The product-state null obeys its bound by 0.675 (73σ separation). **[Finding](findings/h13-cell5-hardy-whisper-c5048.md)**

Five more cells are drafted and Tier-0-cleared behind the tank: the Elitzur–Vaidman bomb-tester with its Zeno ladder, the two-time hindsight meter (gap(θ)=sin θ/2, the trivial-copy ceiling caught at design time), the chip's own emergent light cone (with the first purpose-built constant-vs-weather verdict), the **Causal Compass** flagship (cause vs common-cause from *observation alone* — classically impossible; claim card already clears all four attack classes), and the Deutsch-vs-postselected CTC discrimination court. **[Arc spec](docs/h13-temporal-investigations-whisper-c5048.md) · [Tier-0 design studies](docs/h13-tier0-design-studies-whisper-c5048.md)**
...
## The Quantum Museum — 50 interactive exhibits in 8 wings
...
5 consecutive experiments where pre-submission review caught a real defect · 2 advantage claims retired by our own red team (F119, F121) and 1 WIN earned under the standard their retirement built (door (b), 9.3× at 104σ).
...
- **The court standard (matured through the door arcs, August)**: advantage-class claims carry a **claim card** (floor status: PROVEN-IN-PRINT / DERIVED-OURS / CONDITIONAL / OPEN, floor scale, measured effect) and must clear **`tools/attack_preflight.py`** — a registry of every attack class that has killed one of our own claims, fired automatically at each new one. Flights run as **3-of-3 blind courts** (register / seal+fly / decode as different seats; decisions hashed before any unseal; the submitter never sees a result before the hash posts). **Authorizations are single-use and seal-bound** — a go is consumed by the flight it buys and a re-fly needs a fresh go citing the new seal's digest. Claims evaluate at **delivered** amplitude, never nominal (the delivered-ε clause), and flight budgets are **weather-gated functions of the epoch actually occupied**, frozen before any epoch is known.
